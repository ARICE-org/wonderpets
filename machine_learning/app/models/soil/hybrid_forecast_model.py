...
class HybridSoilForecastModel(BaseMLModel):
    ...
    # Default seasonal baselines for fallback when model is not trained
    ...
    def __init__(
        ...
        # Use default baseline until trained
    ...
    def load(self, model_path: str) -> None:
        ...
        try:
            ...
            if path.exists():
                ...
                # Use loaded baseline if valid, otherwise keep defaults
    ...
    def save(self, model_path: str) -> None:
        ...
        try:
            ...
            logger.info(f"Saved hybrid models to {model_path}")
        except Exception as e:
    ...
    def train(
        ...
        **kwargs
        ...
        # Step 2: Calculate seasonal baselines
        ...
        for param in target_params:
            ...
            # 3. Hybrid model (ML learns residuals)
            train_residuals = y_train - rule_train_pred
            
            hybrid_model = RandomForestRegressor(
                n_estimators=self.ml_config.n_estimators,
                max_depth=max(2, self.ml_config.max_depth - 2),  # Simpler for residuals
                min_samples_split=self.ml_config.min_samples_split,
                min_samples_leaf=self.ml_config.min_samples_leaf,
                random_state=self.ml_config.random_state,
                n_jobs=-1
            ...
            # Hybrid = Rule-based + ML residual correction
    ...
    def predict(
        ...
        Args:
            ...
            use_approach: "hybrid", "rule_based", or "pure_ml"
    ...
    def forecast_season(
        ...
        Args:
            historical_data: Recent soil readings for lag features
            planting_date: Expected planting date
            forecast_days: Number of days to forecast (default 90)
            interval_days: Forecast interval in days
            
        Returns:
            Comprehensive forecast with all three approaches
        """
        ...
        for day_offset in range(0, forecast_days + 1, interval_days):
            ...
            # Generate predictions - only include hybrid values in output
            for param in self.TARGET_PARAMETERS:
                # Calculate all approaches internally but only output hybrid
    ...
    def _generate_forecast_features(
        ...
        # Add lag features from historical data or baseline
        for param in self.TARGET_PARAMETERS:
    ...
    def _compare_approaches(self, forecasts: List[Dict]) -> Dict[str, Any]:
        """Compare predictions from all three approaches."""
        comparison = {}
        
        for param in self.TARGET_PARAMETERS:
            rule_vals = [f.get(f'{param}_rule', 0) for f in forecasts]
            ml_vals = [f.get(f'{param}_ml', 0) for f in forecasts]
            hybrid_vals = [f.get(f'{param}_hybrid', 0) for f in forecasts]
            
            comparison[param] = {
                'rule_based_avg': round(np.mean(rule_vals), 2),
                'pure_ml_avg': round(np.mean(ml_vals), 2),
                'hybrid_avg': round(np.mean(hybrid_vals), 2),
                'ml_correction_avg': round(np.mean(np.array(hybrid_vals) - np.array(rule_vals)), 4)
            }
...
1. RULE-BASED LAYER (Soil Science Knowledge)
   - Applies documented seasonal effects from agricultural literature
   - Provides explainable baseline predictions
   - No training required, uses domain expertise
   - Example: "Nitrogen decreases 10-15% during wet season due to leaching"

2. ML RESIDUAL LAYER (Pattern Learning)
   - Random Forest learns errors/residuals from rule-based predictions
...
ADVANTAGES:
- Explainability: Can explain why predictions were made
- Accuracy: ML corrects rule-based errors
- Robustness: If ML fails, rules still provide reasonable baseline
"""
