...
type MetricItemProps = Omit<SoilMetric, 'id'>;
...
const MetricItem = ({ label, value, unit, status }: MetricItemProps) => {
    ...
    return (<HStack space="sm" alignItems="center" width="50%">
        <Center width={40} height={40} borderRadius="$full" bg={`${iconColor}20`}>
            <Text fontSize="$xl" fontWeight="$bold" color={iconColor}>
                {icon}
            </Text>
        </Center>
        <VStack> <HStack alignItems="flex-end"> <Text fontSize="$xl" fontWeight="$bold" color={iconColor}>
            {value}
        </Text>
            <Text fontSize="$sm" fontWeight="$medium" color={iconColor} mb="$0.5">
                {' '}{unit}
            </Text> </HStack> <Text fontSize="$sm" color="$coolGray600">
                {label} </Text>
        </VStack>
    </HStack>
    );
