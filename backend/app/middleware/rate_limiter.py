...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    def _default_key_func(self, request: Request) -> str:
        # Check for forwarded header (when behind reverse proxy/load balancer)
...
# Convenience function for dependency injection-based rate limiting
...
class RateLimiters:
    """Pre-configured rate limiters for common use cases. Uses environment variables."""
    
    _instances: Dict[str, FixedWindowRateLimiter] = {}
    
    @classmethod
    def standard(cls) -> FixedWindowRateLimiter:
        """Standard API rate limit (default: 100 requests per 60 seconds)"""
    ...
    @classmethod
    def auth(cls) -> FixedWindowRateLimiter:
        """Strict rate limit for auth endpoints (default: 10 requests per 60 seconds)"""
        if "auth" not in cls._instances:
            settings = _get_settings()
            cls._instances["auth"] = FixedWindowRateLimiter(
                requests_per_window=settings.RATE_LIMIT_AUTH_REQUESTS,
                window_size_seconds=settings.RATE_LIMIT_AUTH_WINDOW
            )
        return cls._instances["auth"]
    
    @classmethod
    def relaxed(cls) -> FixedWindowRateLimiter:
        """Relaxed rate limit (default: 1000 requests per 60 seconds)"""
        if "relaxed" not in cls._instances:
            settings = _get_settings()
            cls._instances["relaxed"] = FixedWindowRateLimiter(
                requests_per_window=settings.RATE_LIMIT_RELAXED_REQUESTS,
                window_size_seconds=settings.RATE_LIMIT_RELAXED_WINDOW
            )
        return cls._instances["relaxed"]
    
    @classmethod
    def strict(cls) -> FixedWindowRateLimiter:
        """Very strict rate limit for expensive operations (default: 5 requests per 60 seconds)"""
        if "strict" not in cls._instances:
            settings = _get_settings()
            cls._instances["strict"] = FixedWindowRateLimiter(
                requests_per_window=settings.RATE_LIMIT_STRICT_REQUESTS,
                window_size_seconds=settings.RATE_LIMIT_STRICT_WINDOW
            )
        return cls._instances["strict"]
